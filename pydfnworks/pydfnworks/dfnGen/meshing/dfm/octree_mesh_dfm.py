...
def octree_dfm_driver(num_fracs, h, box_domain, psets):
...
dump / exo / dfm_octree.exo / mo_dfm / psets / / &
     facesets &
"""
        lagrit_script += floop 
        lagrit_script += """
...
dump / exo / dfm_octree.exo / mo_dfm / / / &
     facesets &
"""
        lagrit_script += floop 
        lagrit_script += """
...
"""
    ...
    with open('dfm_octree_mesh.lgi', 'w') as fp:
...
def create_octree_dfm(self, l, allowed_percentage, psets):

    # estimate the number of orl needed to get close to the fracture meshing size
    orl = np.ceil(np.log2(l/(0.5*self.h))).astype(int)
    ...
    octree_dfm_driver(self.num_frac, self.h, box_domain, psets)
